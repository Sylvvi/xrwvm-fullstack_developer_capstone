...
try {
  Reviews.deleteMany({}).then(() => {
    Reviews.insertMany(reviews_data.reviews); // Changed to dot notation
  });
  Dealerships.deleteMany({}).then(() => {
    Dealerships.insertMany(dealerships_data.dealerships); // Changed to dot notation
  });
} catch (error) {
...
app.post('/insert_review', express.raw({ type: '*/*' }), async (req, res) => {
  ...
  let new_id = documents[0].id + 1; // Changed to dot notation
